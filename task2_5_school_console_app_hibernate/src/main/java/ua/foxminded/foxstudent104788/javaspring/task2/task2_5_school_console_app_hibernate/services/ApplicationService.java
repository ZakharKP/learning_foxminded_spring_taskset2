...
import ua.foxminded.foxstudent104788.javaspring.task2.task2_5_school_console_app_hibernate.services.impl.ConsoleUserService;
...
public class ApplicationService implements Runnable {
	...
	private void proceedDelStudentById(UserService userService) {
		...
		int status = studentService.deleteStudent(student.get());
	...
	private void proceedStudentToCourse(UserService userService) {
		...
		int status = studentService.addStudentToCourse(student.get(), course.get());
	...
	private void proceedRemoveStudentFromCourse(UserService userService) {
		...
		int status = studentService.remoweCourse(student.get(), course.get());
